...
<nav class="main-nav navbar navbar-expand-lg navbar-light bg-light p-2">
  ...
  <div class="collapse navbar-collapse" id="navbarNavDropdown">
    <ul class="navbar-nav">
      <li class="nav-item">
        <a class="nav-link <?=($this->controller_name() == 'Home2') ? ' active-nav ':''?> " href="<?=ROOT?>/home2">DASHBOARD</a>
      </li>

      <?php if(Auth::access('super_admin')):?>
        <li class="nav-item">
          <a class="nav-link <?=($this->controller_name() == 'Schools') ? ' active-nav ':''?> " href="<?=ROOT?>/crms">CRMS</a>
        </li>
      <?php endif;?>

      <?php if(Auth::access('admin')):?>
        <li class="nav-item">
          <a class="nav-link <?=($this->controller_name() == 'Users') ? ' active-nav ':''?> " href="<?=ROOT?>/users">USERS</a>
        </li>
      <?php endif;?>

      <?php if(Auth::access('reception')):?>
        <li class="nav-item">
          <a class="nav-link <?=($this->controller_name() == 'Students') ? ' active-nav ':''?> " href="<?=ROOT?>/accounts">ACCOUNTS</a>
        </li>
      <?php endif;?>
      
      <li class="nav-item">
        <a class="nav-link <?=($this->controller_name() == 'Classes') ? ' active-nav ':''?> " href="<?=ROOT?>/classes">CONTACTS</a>
      </li>
      
      <li class="nav-item" style="position: relative;">
        <a class="nav-link <?=($this->controller_name() == 'Tests') ? ' active-nav ':''?> " href="<?=ROOT?>/leads">LEADS
      ...
      <?php if(Auth::access('lecturer')):?>
        <li class="nav-item" style="position: relative;">
          <a class="nav-link <?=($this->controller_name() == 'To_mark') ? ' active-nav ':''?> " href="<?=ROOT?>/opportunities">OPPORTUNITIES
            <?php  
        ...
        <li class="nav-item">
          <a class="nav-link <?=($this->controller_name() == 'Marked') ? ' active-nav ':''?> " href="<?=ROOT?>/marked">CASES</a>
        </li>
      <?php endif;?>

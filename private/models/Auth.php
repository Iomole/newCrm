...
class Auth
{
	...
	public static function switch_crm($id)
	{
		if(isset($_SESSION['USER']) && $_SESSION['USER']->rank == 'super_admin')
		{
			...
			if($row = $crm->where('id',$id))
			{
				$row = $row[0];
 				$arr['crm_id'] = $row->crm_id;

				$user->update($_SESSION['USER']->id,$arr);
 				$_SESSION['USER']->crm_id = $row->crm_id;
				$_SESSION['USER']->crm_name = $row->crm;
	...
	public static function access($rank = 'student')
	{
		...
		$RANK['super_admin'] 	= ['super_admin','admin','lecturer','reception','student','presales','sales'];
		$RANK['admin'] 			= ['admin','lecturer','reception','student','sales','presales'];
		$RANK['lecturer'] 		= ['lecturer','reception','student'];
		$RANK['presales'] 		= ['presales','lecturer','reception','student','sales'];
		$RANK['sales'] 		    = ['sales','reception','student'];

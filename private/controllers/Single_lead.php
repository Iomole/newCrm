 ...
 */
class Single_lead extends Controller
{
	...
	public function index($id = '')
	{
		...
		$page_tab = isset($_GET['tab']) ? $_GET['tab'] : 'assigned_users';
		$lect = new Assigned_users_model();

		$results = false;

		if($page_tab == 'assigned_users'){
			
			//display assigned users
			$query = "select * from lead_contacts where lead_id = :lead_id && disabled = 0 order by id desc limit $limit offset $offset";
			$assigned_users = $lect->query($query,['lead_id'=>$id]);
	...
	public function lectureradd($id = '')
	{
		...
		if(count($_POST) > 0)
		{
			...
			if(isset($_POST['selected'])){
				...
				//add lecturer
				$query = "select disabled,id from lead_assignedusers where user_id = :user_id && class_id = :class_id limit 1";
				...
				if(!$check = $lect->query($query,[
					...
					$this->redirect('single_lead/'.$id.'?tab=assigned_users');
					...
					if(isset($check[0]->disabled))
					{
						if($check[0]->disabled)
						{
							...
							$errors[] = "that user is already assigned to this lead";
						}
					}else{
						$errors[] = "that user is already assigned to this lead";
					}
...
public function assignedUseradd($id = '')
	{
		...
		if(count($_POST) > 0)
		{
			...
			if(isset($_POST['selected'])){
				...
				//add assigned user
				$query = "select disabled,id from lead_assignedusers where user_id = :user_id && lead_id = :lead_id limit 1";
